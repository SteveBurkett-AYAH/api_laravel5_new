...
class CreateClientReportByPublisherCampaign extends Migration {
	...
	public function up()
	{
		Schema::create('ClientReportByPublisher_campaign', function(Blueprint $table)
		{
			$table->date('date_logged');
            $table->string('range_type', 100);
            $table->char('publisher_key', 40);
            $table->char('campaign_id', 255);
            ...
            $table->unique(['date_logged', 'range_type', 'publisher_key', 'campaign_id'], 'clientreportbypublisher_campaign_uniquekey');
		});
	}
	...
	{
		Schema::drop('ClientReportByPublisher_campaign');
	}
